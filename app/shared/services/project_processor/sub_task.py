...
class SubTask:
    ...
    listener: Callable[[SubTaskLog, SubTask],
    ...
    async def start(self) -> None:
        if self._status != ProcessingStatus.pending:
            raise RuntimeError(
                f'Cannot process already started file {self.id}'
            )

        self._status = ProcessingStatus.processing
        await self._log(f'File {self.file.file_name} started')

        self.result = await STTRepo.instance.transcribe_from_sss_path(self.file.file_path_raw)
        ...
        if self.result.status_code == 201:
            ...
            self._error_msg = msg
            await self._err(f'File {self.file.file_name} failed', code=self.result.status_code)
